...
cat("\f")
rm(list=ls())
setwd("~/Downloads/Universidad/Taller de R/Clases/clase_3") # Cambiar este directorio
...
## La libreria Haven sirve para leer bases de datos en formato de stata
## De igual forma Haven permite exportar bases de datos a ese mismo formato
...
## En caso de no seleccionar toda la linea sin cerrar el parentesis,
## la consola qued  a esperando el resto del input
...
## Esta funcion nos arroja las hojas de excel que contiene el archivo
?read_excel
...
# Funcion %>% (Pipe)
## El operador %>% nos permite utilizar varias funciones en la misma linea.
## Actua como algo adicional del codigo
## Este operador tambien facilita la lectura del archivo

c("2","4") %>% as.numeric(.)
## El punto le dice a R que tenga en cuenta los argumentos utilizados anteriormente
...
'Vamos a seleccionar solo algunas columnas del dataframe data_csv'
View(data_csv)
...
## Como sabemos que algunos datos son na, se utiliza esto para filtrar la base de datos
